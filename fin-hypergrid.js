...
(function() {
    ...
    Polymer({ /* jslint ignore:line */
        ...
        createDragColumn: function(x, colIndex) {
            ...
            style.webkitTransform = 'translate(' + x + 'px, 0px)';
            style.zIndex = '5';
            style.cursor = 'none';
            this.repaint();
        ...
        dragColumn: function(x) {
            ...
            noop(autoScrollingNow);
            if (this.autoScrollingNow) {
                return;
            }
            ...
            d.style.webkitTransform = 'translate(' + x + 'px, ' + -10 + 'px)';
            d.style.display = 'inline';
            ...
            var doAFloat = dragColumnIndex > overCol && (dragColumnIndex - overCol > 0);
            doAFloat = doAFloat || (overCol - dragColumnIndex > 1);
            //var colDif = overCol - dragColumnIndex;
            //var draggedToTheRight = colDif > 1;
            //        var threshold = draggedToTheRight ? 1 : 0;
            //        var isToFar = overCol > colCount;
            if (doAFloat && !this.floatingNow && !atMax) {
        ...
        _checkAutoScrollToRight: function(x) {
            ...
            //var cw = this.renderer.renderedColWidths;
            //var removeColWidth = behavior.getColWidth(0);
            //var addColWidth = cw[cw.length - 1] - removeColWidth - this.columnRenderOverridesCache.dragger.width;
            //var csx = this.columnRenderOverridesCache.dragger.startX;

            behavior.swapColumns(draggedIndex + scrollLeft + 1, draggedIndex + scrollLeft);
            this.scrollBy(1, 0);

            //this.columnRenderOverridesCache.dragger.startX = ;
            //var startX = this.renderer.renderedColWidths[draggedIndex + numFixedCols];

            //this.columnRenderOverridesCache.dragger.startX = startX;
        ...
        endDragColumn: function() {
            ...
            var startX = this.renderer.renderedColWidths[colIndex + numFixedCols];
            var d = this.dragger;
            d.style.webkitTransition = '-webkit-transform ' + colAnimationTime + 'ms ease, box-shadow ' + colAnimationTime + 'ms ease';
            d.style.transition = 'transform ' + colAnimationTime + 'ms ease, box-shadow ' + colAnimationTime + 'ms ease';
            d.style.webkitTransform = 'translate(' + startX + 'px, ' + -1 + 'px)';
