...
var JSON = DataModel.extend('dataModels.JSON', {
    ...
    initialize: function(grid, options) {
        /**
         * @summary Hash of DCIs (data control interfaces).
         * @desc Keyed by data source type. An DCI is accepted by data sources with an `type` property.
         * @see {@link dataModels.JSON/updateDataSources}
         * @type {dataControlInterface[]}
    ...
    setPipeline: function(DataSources, options) {
        ...
        this.setController(options.controllers || this.controllers); // set the new or previously set DCIs on the new pipeline
    },
    ...
    /**
     * @summary Get the given DCI.
     * @param {string} type
     * @returns {undefined|*} The DCI; or `undefined` if DCI unknown to data model.
     * @memberOf DataModel#
     */
    getController: function(type) {
        return this.dataSource.getController(type);
    },

    /**
     * @summary Set the given DCI(s).
     * @param {string} natypeme
     * @param {*} dci
     * @param {string} hash - One of:
     * * **object** - Hash of multiple DCIs, by type.
     * * **string** - Type of a single DCI (given in `dci`).
     * @param {*} [dci] - Only required when 'hash' is a string; omit when `hash` is an object.
     * @returns {object} - Hash of all results, by type. Each member will be:
     * * When the given DCI for that type is defined (_i.e.,_ not `undefined`)
     * * when the given DCI for that type was `undefined`A new (null) DCI generated by the data model ; or `undefined` if the DCI was unknown to the data model.
     * @memberOf DataModel#
     */
    setController: function(typeOrHashOfTypes, dci) {
    ...
    /**
     * @summary Digests `(columnIndex, propName, value)` and calls specified DCI's `properties()` method.
     * @desc Digests the three parameters `(columnIndex, propName, value)` detailed below, creating a single {@link dataControlInterface} object with which it then calls the `properties` method of the DCI specified by `type`.
     *
     * This method is overloaded in the jQuery style: You can both set a data DCI prop (when value give) and a get a data DCI prop (when value omitted); or you can give a hash in place of the property name to set several properties at once. Whichever way you use it, you can in addition specify a column index for column-specific properties.
     *
     * @param {string} type
     *
     * @param {number} [columnIndex] - If given, this is a property on a specific column. If omitted, this is a property on the whole DCI properties object.
     *
    ...
    prop: function(type, columnIndex, propNameOrPropHash, value) {
        ...
        if (controller) {
            ...
            return controller.properties(properties);
        }
    },
