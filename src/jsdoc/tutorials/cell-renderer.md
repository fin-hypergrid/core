...
Data will exist in your app in different forms.

#### Raw data
For the purpose of fetching and persisting a cell's value, the underlying data backing each cell is serialized text data, reflecting the JSON primitive types (string literals, a numeric constants, `true`, `false`, or `null`).
 
#### Formatted text data
The default cell renderer renders data as text. The text may or may not be formatted. In the absence of a formatter, the renderer converts the data to string format (using the implicit `toString()`). The default renderer will respect the cell's `format` render property, invoking the formatter on the named localizer to render the text into a more human-friendly form. For example, if the raw data is 123456.789123, it might be rendered as "123,466"; the precise format is of course up to the formatter.

#### Localized text data
As soon as you start formatting data, you run into the various local differences for doing so. Hypergrid defines a lightweight _localization_ API for creating and registering _localizers._ Localizers format raw data into human-friendly form, "deformat" human-edited data back into raw form, and may also include validation logic. Localizers have a `locale` property. Your custom formatters should respect this locale when feasible. The localizers generated by the included factory functions to this (via the `Intl` API). See the separate {@tutorial localization} tutorial for more information.

#### Infographics

All cells are assigned a _cell renderer_ which renders the data into the cell. While cells typically render as text using the default cell renderer, they are not constrained to do so. Cells may specify other cell renderers that render the data as _infographics._ Examples include {@link https://en.wikipedia.org/wiki/Sparkline|sparklines} (inline chart), a {@link https://en.wikipedia.org/wiki/Star_(classification)|star ratings}, and {@link https://en.wikipedia.org/wiki/Error_bar|confidence intervals}. Implementations of all of these are included with Hypergrid. You can also write your own custom cell renderers and assign them to particular cells. See the separate {@tutorial cell-renderer} tutorial for more information.
