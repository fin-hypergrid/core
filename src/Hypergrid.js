...
var Hypergrid = Base.extend('Hypergrid', {
    ...
    /**
     *
     * @param {number|string} [space='\t'] - See {@link https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify|JSON.stringify}'s `space` param
     * @param {function} [headerify] - If your headers were generated by a function (taking column name as a parameter), give a reference to that function here to avoid persisting headers that match the genereated string
     */
    saveState: function(space, headerify) {
        var properties = this.properties;
        if (properties.calculators) {
            properties.calculators.toJSON = stringifyFunctions;
        }
        if (space === undefined) {
            space = '\t';
        }
        ...
        Object.keys(dynaPropDescriptors).forEach(function(key) { delete properties[key]; });
