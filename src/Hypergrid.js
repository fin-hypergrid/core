...
var Hypergrid = Base.extend('Hypergrid', {
     ...
     * @param {string} type
     * @returns {null|undefined|*} The DCI or:
     * * `null` means unknown DCI.
     * * `undefined` means the data source handles this DCI but the DCI is undefined.
     * @memberOf Hypergrid#
     */
     ...
     * @desc The data model needs to be able to accept the specified DCI type(s). If it fails to accept the specified DCI(s), an error condition is raised.
     *
     ...
     *
     * Setting one or more DCIs triggers a shape change.
     ...
     * @returns {object} - Hash of all results, by DCI. Each will be the given DCI for each; or a null DCI generated by data model if the given DCI was `undefined`; or `undefined` if the DCI was unknown to the data model.
     * @memberOf Hypergrid#
     */
    setController: function(typeOrHashOfTypes, dci) {
        var results = this.behavior.setController(typeOrHashOfTypes, dci),
            rejections = Object.keys(results).filter(function(name) {
                return !results[name];
            }).toString();

        if (rejections) {
            throw 'Unexpected DCI(s): ' + rejections;
        }
