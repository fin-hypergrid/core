...
var isValidIdentifer = function(string) {
...
that.setData = function(jsonData) {
    that.data = jsonData;
    that.initDataIndexes();
};

that.initDataIndexes = function() {
...
that.fetchTableData = function(data) {
    that.lastFetch = data;
    var result = new Array(data.num);
    for (var i = 0; i < data.num; i++) {
        result[i] = that.data[data.start + i];
    }
    postMessage({
        data: result,
        headers: that.getHeaders(),
        fields: that.getFields(),
        rows: that.data.length,
        features: {
            sorting: true
        }
    });
...
that.setHeaders = function(headerLabels) {
    that.headers = headerLabels;
};

that.getHeaders = function() {
    if (that.headers.length === 0) {
        that.setHeaders(that.getDefaultHeaders());
    }
    return that.headers;
};
that.getHeader = function(x /*, y*/ ) {
    return that.headers[x];
};

that.getDefaultHeaders = function() {
    var self = that;
...
that.headerify = function(string) {
...
that.setFields = function(fieldNames) {
    that.fields = fieldNames;
};

that.getFields = function() {
    if (that.fields.length === 0) {
        that.setFields(that.getDefaultFields());
    }
    return that.fields;
};

that.getDefaultFields = function() {
    if (that.data && that.data.length === 0) {
        return [];
    }
    var fields = Object.getOwnPropertyNames(that.data[0]).filter(function(e) {
        return e.substr(0, 2) !== '__';
    });
    return fields;
};

that.sortTable = function(data, incrementIt) {

    var columnIndex = data.sortIndex;
    var fields = that.getFields();
    var sortStateIndex = data.state;
    var i;
    ...
    if (sortStateIndex === 0) {
        var newData = new Array(that.data.length);
        for (i = 0; i < that.data.length; i++) {
            var each = that.data[i];
            newData[each.__si] = each;
        }
        that.data = newData;
    } else if (sortStateIndex === 1) {
        if (isValidIdentifer(colName)) {
...
that.changed = function() {
    that.fetchTableData(that.lastFetch);
};

onmessage = function(msg) { /* jslint ignore:line */
    var data = msg.data;
    that[data.cmd](data.data);
};
